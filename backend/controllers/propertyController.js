...
propertyController.get('/find/types', async (req, res) => {
    try {
        const oneBHKType = await Property.countDocuments({ type: '1BHK' })
        const twoBHKType = await Property.countDocuments({ type: '2BHK' })
        const oneRKType = await Property.countDocuments({ type: '1RK' })
        const hostelType = await Property.countDocuments({ type: 'HOSTEL' })
        const PGtype = await Property.countDocuments({ type: 'PG' })

        return res.status(200).json({ onebhk: oneBHKType, twobhk: twoBHKType, onerk: oneRKType, hostel : hostelType, pg : PGtype })
    } catch (error) {
        return res.status(500).json(error)
    }
})
